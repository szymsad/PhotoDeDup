...
class Database:
    ...
    def get_cached(self, path: str, size: int, mtime: float) -> Optional[CachedFile]:
        ...
        row = self._conn.execute(
            "SELECT path, size, mtime, md5, phash, width, height, exif_date, exif_make, exif_model "
    ...
    def upsert(self, file: CachedFile):
        """Zapisuje lub aktualizuje wynik skanowania pliku."""
        from datetime import datetime
        self._conn.execute("""
            INSERT INTO files
                (path, size, mtime, md5, phash, width, height, exif_date, exif_make, exif_model, scanned_at)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    ...
    def remove_missing(self, existing_paths: set[str]):
        ...
        cached_paths = {row[0] for row in self._conn.execute("SELECT path FROM files").fetchall()}
        to_remove = cached_paths - existing_paths
        if to_remove:
            self._conn.executemany("DELETE FROM files WHERE path=?", [(p,) for p in to_remove])
            self._conn.commit()
    ...
    def stats(self) -> dict:
        ...
        with_hash = self._conn.execute("SELECT COUNT(*) FROM files WHERE md5 IS NOT NULL").fetchone()[0]
